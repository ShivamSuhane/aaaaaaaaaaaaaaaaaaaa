...
export function Settings() {
  ...
  const handleSoundChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setAppSettings(prev => ({ ...prev, sound: e.target.value }));
    showToast(`Sound changed to ${e.target.value}`, 'success');
  };

  const handleResetTimeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setAppSettings(prev => ({ ...prev, autoResetTime: e.target.value }));
    showToast(`Auto-reset time set to ${e.target.value}`, 'success');
  };

  const handleDefaultLandingChange = (value: 'dashboard' | 'defaultMantra') => {
    if (value === 'defaultMantra' && !defaultMantra) {
      showToast('Please set a default mantra first from Add Mantra page', 'warning');
      return;
    }
    setAppSettings(prev => ({ ...prev, defaultLandingPage: value }));
      ...
      <div className="flex-1 overflow-auto">
            ...
            >
              ...
              <div className="text-left">
                ...
                <span className="block text-xs text-gray-500">
                  {defaultMantra ? `Current: ${defaultMantra.name}` : 'No default mantra set'}
                </span>
              </div>
            </button>
          </div>
        ...
        <div className="bg-white dark:bg-slate-800 border-b border-gray-100 dark:border-slate-700 mt-2">
          ...
          <div className="px-4 py-2">
            ...
            <div className="flex items-center justify-between py-2">
              ...
              <select value={appSettings.sound} onChange={handleSoundChange} className="px-3 py-1.5 border-2 border-gray-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-800 dark:text-white text-sm">
        ...
        <div className="bg-white dark:bg-slate-800 border-b border-gray-100 dark:border-slate-700 mt-2">
          ...
          <div className="px-4 py-2">
            ...
            <div className="flex items-center justify-between py-2">
              ...
              <input type="time" value={appSettings.autoResetTime} onChange={handleResetTimeChange} className="px-3 py-1.5 border-2 border-gray-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-800 dark:text-white text-sm" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 mt-2 mb-4">
          ...
          <div className="px-4 py-3 text-center">
            ...
            <p className="text-xs text-gray-500 mt-2">Built with React + Firebase + Tailwind</p>
          </div>
        </div>
      </div>
    </div>
